...
| `onError`           | logs via `console.error` | Called on every `'error'` event from the underlying `Queue`/`Worker`.                |
...
| `connection`         | — (required if no `url`)        | An already-open connection you own, or any wrapper whose `createChannel()` **and** `createConfirmChannel()` resolve to real amqplib `Channel`/`ConfirmChannel`; the driver never closes it. |
...
| `onError`            | logs via `console.error`        | Called on every `'error'` event from the connection or this driver's channels, and for teardown failures and oversized messages.                                                            |

Delivery is **at-least-once** on both sides. **Producing:** `enqueue()` publishes on a confirm channel
(`createConfirmChannel()`) and resolves only once the broker has acknowledged the message, rejecting
with a `MediaLibraryError` if the broker nacks it — a plain channel's `sendToQueue()` is fire-and-forget,
so this is what makes an awaited `enqueue()` actually mean "the broker has it," not just "it reached a
socket buffer." **Consuming:** a crash between a processor's side effects and the broker receiving the
...
forever. A consumed message over 64 KiB is rejected unparsed and reported through `onError`.
...
`ChannelWrapper`, not a `Promise<Channel>` (and it has no `createConfirmChannel()` at all). Run the
worker under a supervisor and exit deliberately from `onError` when the connection drops. See the
package's
