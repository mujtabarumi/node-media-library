...
describe('definition builders', () => {
  it('collection builder produces plain serializable data', () => {
    ...
    expect(def.conversions.thumb).toMatchObject({
      width: 368,
      height: 232,
      fit: 'cover',
      queued: false,
  ...
  it('conversion defaults: queued, keep-original format', () => {
    ...
    expect(def.queued).toBe(true)
    expect(def.format).toBeNull()
  })
  ...
  it('conversions() still accepts a normal name', () => {
    const def = collection()
      .conversions({ thumb: conversion().width(100) })
      .toDefinition()
